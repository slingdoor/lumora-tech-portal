...
export default function DashboardPage() {
  ...
  const stats = [
    { label: t('stats.activeChatbots'), value: '12', change: '+3', icon: Bot, color: 'text-blue-400' },
    { label: t('stats.queriesResolved'), value: '2,847', change: '+12%', icon: MessageSquare, color: 'text-purple-400' },
    { label: t('stats.activeUsers'), value: '1,429', change: '+8%', icon: Users, color: 'text-blue-400' },
    { label: t('stats.efficiencyGain'), value: '87%', change: '+5%', icon: TrendingUp, color: 'text-green-400' },
  ];

  const chatbots = [
    { name: 'HR Assistant', status: t('chatbots.status.active'), queries: 342, accuracy: '95%' },
    { name: 'IT Support Bot', status: t('chatbots.status.active'), queries: 189, accuracy: '92%' },
    { name: 'Sales Knowledge', status: t('chatbots.status.training'), queries: 67, accuracy: '88%' },
    { name: 'Legal Advisor', status: t('chatbots.status.active'), queries: 234, accuracy: '97%' },
  ];
      ...
      <div className="lg:ml-64">
        ...
        <header className="glass border-b border-blue-500/20 h-16 flex items-center justify-between px-6">
          ...
          <div className="flex items-center space-x-4">
            <span className="text-gray-300">{t('welcome')}</span>
        ...
        <main className="p-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-2">
              ...
              >
                <h2 className="text-xl font-semibold text-white mb-6">{t('chatbots.title')}</h2>
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-400">
                            <span className={`inline-block w-2 h-2 rounded-full mr-2 ${bot.status === t('chatbots.status.active') ? 'bg-green-400' : 'bg-yellow-400'}`}></span>
                            {bot.status}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-white font-medium">{bot.queries} {t('chatbots.queries')}</p>
                        <p className="text-sm text-blue-400">{bot.accuracy} {t('chatbots.accuracy')}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div>
              ...
              >
                <h2 className="text-xl font-semibold text-white mb-4">{t('recentActivity.title')}</h2>
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    <div className="w-2 h-2 bg-green-400 rounded-full"></div>
                    <p className="text-sm text-gray-300">{t('recentActivity.activities.resolved')}</p>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-2 h-2 bg-blue-400 rounded-full"></div>
                    <p className="text-sm text-gray-300">{t('recentActivity.activities.updated')}</p>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-2 h-2 bg-purple-400 rounded-full"></div>
                    <p className="text-sm text-gray-300">{t('recentActivity.activities.completed')}</p>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-2 h-2 bg-yellow-400 rounded-full"></div>
                    <p className="text-sm text-gray-300">{t('recentActivity.activities.joined')}</p>
                  </div>
                </div>
              </motion.div>
            </div>
